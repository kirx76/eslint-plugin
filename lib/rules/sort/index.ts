      ...
      Program({ body }: Program) {
        body
          ...
          .map((node) => {
            ...
            const isExportableDeclaration = (): boolean => ['ExportNamedDeclaration', 'ExportDefaultDeclaration'].includes(
              node.type,
            );

            if (isExportableDeclaration()) {
              declarations = node.declaration?.declarations?.[0].init
                || node.declaration;
            } else {
          ...
          .forEach((declarations: Node[]) => {
            ...
            const hooks = nodes
              ?.map(
                ({ type, callee, init }) => (type === 'CallExpression'
                  ? [type, callee]
